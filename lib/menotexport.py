...
import argparse
import pandas as pd
import lib.extracttags as extracttags
import lib.extractnt as extractnt
import lib.exportpdf as exportpdf
import lib.exportannotation as exportannotation
import lib.export2bib as export2bib
import lib.export2ris as export2ris
...
def extractAnnos(annotations,action,verbose):
	...
	for ii,idii in enumerate(docids):
		...
		highlight_text_list.append(hltexts)
		
		#print(hltexts)
		#break
		#if ii==1:
		#	break
	return annotations2,faillist,highlight_text_list
...
def processFolder(db,outdir,annotations,folderid,foldername,allfolders,action,\
	...
	if len(annotations)>0:
		...
		annotations,flist,hl_text=extractAnnos(annotations,action,verbose)
	...
	if ('m' in action or 'n' in action) and len(annotations)>0:
		...
		flist,fl_text=exportannotation.exportAnno(annotations,outdir_folder,action,\
		                                  separate,verbose)
		annofaillist.extend(flist)
...
def processCanonicals(db,outdir,annotations,docids,allfolders,action,\
	...
	if len(annotations)>0:
		if verbose:
			printHeader('Extracting annotations from PDFs ...',2)
		annotations,flist,highlight_text_list_single=extractAnnos(annotations,action,verbose)
		annofaillist.extend(flist)
		#**************************
		highlight_text_list=highlight_text_list_single
		#**************************
	...
	if ('m' in action or 'n' in action) and len(annotations)>0:
		...
		if verbose:
			printHeader('Exporting annotations to text file...',2)
		flist,ret=exportannotation.exportAnno(annotations,outdir_folder,action,separate,verbose)
		annofaillist.extend(flist)
		#**************************
	...
	return exportfaillist,annofaillist,bibfaillist,risfaillist,ret,highlight_text_list
...
def main(dbfin,outdir,action,folder,separate,iszotero,verbose=True):
	...
	if folder is None and len(canonical_doc_ids)>0:
		...
		annotations={}
		exportfaillistii,annofaillistii,bibfaillistii,risfaillistii,highlight_text_list,highlight_text_list2=processCanonicals(db,outdir,annotations,\
